...
/*
 * SDL begin_code.h uses #pragma pack(push,4) on Win32, then SDL_syswm.h
 * includes <windows.h>. Windows SDK 10.0.18362+ asserts
 * TYPE_ALIGNMENT(LARGE_INTEGER)==8, which fails under pack(4) (C2118 in
 * winnt.h). Include windows.h first with default packing so the later
 * include is skipped by the header guard.
 */
#if defined(_WIN32)
#   pragma pack(push, 8)
#   include <windows.h>
