/* $Id: stream.c 5845 2018-07-26 07:19:39Z nanang $ */
/*
...
/* RFC 2833 DTMF transmission queue: */
	...
	pj_bool_t			echo_to_remte;			/* echo rtp to remote but audio driver*/
};
	...
	pjmedia_frame *frame)
{
	...
	if (stream->enc_buf != NULL) {
		...
		//{{ -ddh 2018-12-11 11:07:12
		//return put_frame_imp(port, frame);
		//if (!port->info.echo_from_remote)
		{
			return put_frame_imp(port, frame);
		}
		//}}
...
static void on_rx_rtp(pjmedia_tp_cb_param *param)
{
	...
	if (seq_st.status.flag.restart) {
		...
		for (i = 0; i < count; ++i) {
			...
			//{{ -ddh 2018-12-11 10:31:10
		/*	if (stream->echo_to_remte)
			{
				if (put_frame(&(stream->port), &frames[i]) == PJ_SUCCESS)
					pkt_discarded = PJ_TRUE;
			}
			else
			{
...

{
	...
	pjmedia_port_info_init(&stream->port.info, &name,
		...
		16, 80);
	stream->port.info.echo_from_remote = info->echo_to_remote;
	...
	stream->rtcp_fb_nack.pid = -1;
	stream->echo_to_remte = info->echo_to_remote;
#if defined(PJMEDIA_STREAM_ENABLE_KA) && PJMEDIA_STREAM_ENABLE_KA!=0
