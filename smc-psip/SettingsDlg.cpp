...
BOOL SettingsDlg::OnInitDialog()
{
	...
	for (unsigned i = 0; i < count; i++)
	{
			POSITION pos = mainDlg->GetAudioCodecList().Find(
				PjToStr(&codec_info[i].codec_id)
			);
			CString key = mainDlg->GetAudioCodecList().GetNext(pos);
			CString value = mainDlg->GetAudioCodecList().GetNext(pos);
	...
	POSITION pos = mainDlg->GetAudioCodecList().GetHeadPosition();
	while (pos) {
		CString key = mainDlg->GetAudioCodecList().GetNext(pos);
		CString value = mainDlg->GetAudioCodecList().GetNext(pos);
	...
	((CButton*)GetDlgItem(IDC_SETTINGS_ECHO_TO_REMOTE))->SetCheck(accountSettings.echoToRemote);
...
LRESULT SettingsDlg::OnUpdateSettings(WPARAM wParam, LPARAM lParam)
{
	...
	for (unsigned i = 0; i < listbox2->GetCount(); i++)
	{
		...
		POSITION pos = mainDlg->GetAudioCodecList().Find(value);
		if (pos) {
			mainDlg->GetAudioCodecList().GetPrev(pos);
			CString key = mainDlg->GetAudioCodecList().GetPrev(pos);
	...
	accountSettings.echoToRemote = ((CButton*)GetDlgItem(IDC_SETTINGS_ECHO_TO_REMOTE))->GetCheck();
	...
	{
		mainDlg->GetDialer()->RebuildButtons();
	}
	mainDlg->PJCreate();
