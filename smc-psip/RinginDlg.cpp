...
BOOL RinginDlg::OnInitDialog() {
	...
	if (accountSettings.randomAnswerBox) {
		...
		if (mainDlg->GetRinginDlgs().GetCount())
		{
			CRect rect;
			mainDlg->GetRinginDlgs().GetAt(mainDlg->GetRinginDlgs().GetCount()-1)->GetWindowRect(&rect);
			x=rect.left+22;
			y=rect.top+22;
		} else {
...
{
	int count = mainDlg->GetRinginDlgs().GetCount();
	for (int i = 0; i < count; i++ )
	{
		if ( call_id == mainDlg->GetRinginDlgs().GetAt(i)->call_id)
		{
			if (!accept) {
			...
			mainDlg->GetRinginDlgs().RemoveAt(i);
			DestroyWindow();
			break;
		}
	}
...
void RinginDlg::OnBnClickedDecline()
{
	...
	if (pjsua_call_hangup(call_id, 0, NULL, NULL) == PJ_SUCCESS) {
		mainDlg->SetCallIdIncomingIgnore( PjToStr(&call_info.call_id));
	}
	Close();
}
