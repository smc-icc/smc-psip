...
{
	if (!mainDlg->IsShortcutsEnabled()) {
		return;
	}
	CRect rect;
	if (!init) {
		mainDlg->GetWindowRect(rect);
		mainDlg->SetWindowPos(NULL, 0, 0, mainDlg->GetWindowSize().x, mainDlg->GetWindowSize().y, SWP_NOZORDER | SWP_NOMOVE);
		//--		
...
{
	if (mainDlg->GetWidthAdd() || mainDlg->GetHeightAdd()) {
		SetWindowPos(NULL, 0, 0, lpCreateStruct->cx + mainDlg->GetWidthAdd(), lpCreateStruct->cy + mainDlg->GetHeightAdd(), SWP_NOMOVE | SWP_NOZORDER);
	}
...
void Dialer::DTMF(CString digits)
{
	...
	MessagesContact*  messagesContact = mainDlg->GetMessagesDlg() ? mainDlg->GetMessagesDlg()->GetMessageContact() : NULL;
...
void Dialer::OnBnClickedHold()
{
	...
	//{
	//	mainDlg->GetMessagesDlg()->OnBnClickedHold();
	//}										 
}
