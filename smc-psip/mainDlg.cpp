...
static void on_log_call_back(int level, const char *data, int len)
{
	if (g_log_ptr)
	{
		g_log_ptr->WriteLogEx((SMC_PSIP::LOGLEVEL)level, true, data, len);
	}
}
...
static void on_reg_state2(pjsua_acc_id acc_id, pjsua_reg_info *info)
{
	...
	{
		PostMessage(mainDlg->m_hWnd, UM_ON_REG_STATE2, (WPARAM)info->cbparam->code, (LPARAM)acc_id);
	}
	else
	{
		...
		PostMessage(mainDlg->m_hWnd, UM_SET_MULTIPLE_REGISTER_STATE, (WPARAM)acc_id, (LPARAM)pStatus);
	}
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	...
	{
		CallStatus* pStatus = new CallStatus;
		pStatus->operate = EU_CALL_OPR_CALLOUT;
		pStatus->code = call_info->state;
		switch (call_info->state) {
		case PJSIP_INV_STATE_CALLING:
			pStatus->info.Format(_T("%s..."), Translate(_T("Calling")));
			break;
		case PJSIP_INV_STATE_INCOMING:
			pStatus->info.SetString(Translate(_T("Incoming Call")));
			break;
		case PJSIP_INV_STATE_EARLY:
			pStatus->info.SetString(Translate(PjToStr(&call_info->last_status_text).GetBuffer()));
			break;
		case PJSIP_INV_STATE_CONNECTING:
			pStatus->info.Format(_T("%s..."), Translate(_T("Connecting")));
			break;
		case PJSIP_INV_STATE_CONFIRMED:
			pStatus->info.SetString(Translate(_T("Connected")));
			break;
		case PJSIP_INV_STATE_DISCONNECTED:
			//--
			if (call_info->last_status == 200) {
				pStatus->info.SetString(Translate(_T("Call Ended")));
			}
			else {
				...
				if (call_info->last_status) {
					pStatus->info.Format(_T("%d %s"), call_info->last_status, rab);
				}
				else {
					pStatus->info.SetString(rab);
				}
			}
			break;
		}

		PostMessage(mainDlg->m_hWnd, UM_SET_MULTIPLE_CALL_STATE, (WPARAM)call_id, (LPARAM)pStatus);
	}
}
...
LRESULT CmainDlg::onCallState(WPARAM wParam, LPARAM lParam)
{
	...
	if (call_info->state == PJSIP_INV_STATE_DISCONNECTED) {
		//if (pageCalls)
		//{ 
		//	pageCalls->SetDuration(call_info->acc_id, call_info->connect_duration.sec);
		//}
		//if (call_info->last_status != 200) {
		//	pageCalls->SetInfo(call_info->acc_id, *str);
		//}
	}
	...
	pjsip_rx_data *rdata)
{
	...
	if (IsWindow(mainDlg->m_hWnd)) {
							...
							if (parseStatus == REPARSE_ERROR_OK) {
								...
								if (regex.Match(callInfoValue, &mc) && mc.m_uNumGroups == 1) {
									...
									if (!autoAnswerTimeout) {
										...
										if (mainDlg->GetAutoAnswerCallId() == PJSUA_INVALID_ID) {
											noRingDialog = true;
											mainDlg->SetAutoAnswerCallId(call_id);
		...
		{
			mainDlg->PostMessage(UM_MULTIPLE_INCOMING_CALL, call_id);
		}
	}
}
						...
						for (unsigned i = 0; i < count; i++) {
							if (pjsua_acc_get_config(ids[i], pool, &acc_cfg) == PJ_SUCCESS) {
								...
								if (pjsua_acc_modify(ids[i], &acc_cfg) == PJ_SUCCESS) {
									...
									message.Append(_T(" For more info visit smc-psip website, help page."));
...
void CmainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MAIN_START, m_ButtonStart);
...
BEGIN_MESSAGE_MAP(CmainDlg, CBaseDialog)
	...
	ON_BN_CLICKED(IDC_MAIN_START, OnBnClickedStart)
	ON_BN_CLICKED(IDC_MAIN_STOP, OnBnClickedStop)
	...
	ON_MESSAGE(UM_SET_MULTIPLE_REGISTER_STATE, onSetMultipleRegisterStatus)
	ON_MESSAGE(UM_SET_MULTIPLE_CALL_STATE, onSetMultipleCallStatus)
	ON_MESSAGE(UM_MULTIPLE_INCOMING_CALL, onMultipleIncomingCall)
...
BOOL CmainDlg::OnInitDialog()
{
	...
	if (widthAdd) {
		...
		m_ButtonStart.SetWindowPos(NULL, pageRect.left + widthAdd, pageRect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

		m_ButtonStop.SetWindowPos(NULL, pageRect.left + widthAdd, pageRect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
		//--
	...
	else
	{
		...
		pageCalls->OnCreated();
		tabItem.pszText = Translate(_T("MultipleCallTest"));
		tabItem.iImage = 1;
...
void CmainDlg::OnCreated()
{
	...
	if (!accountSettings.singleMode)
	{
		UpdateWindowText(Translate(_T("Ready")), IDI_ONLINE);
	}
}
...
LRESULT CmainDlg::onCreateRingingDlg(WPARAM wParam, LPARAM lParam)
{
	...
	CString name = !sipuri.name.IsEmpty() ? sipuri.name : (!sipuri.user.IsEmpty() ? sipuri.user : sipuri.domain);
...
void CmainDlg::PJCreate()
{
	...
	*/
	...
	media_cfg.echo_to_remote = accountSettings.echoToRemote;
	...
	if (accountSettings.enableLog) {
		...
		log_cfg.cb = on_log_call_back;
		log_cfg.decor = PJ_LOG_HAS_LEVEL_TEXT;
		//--
	...
	else
	{
		if (mainDlg->GetCalls())
		{
			for (CallsMapIt it = mainDlg->GetCalls()->GetCallMap().begin(); it != mainDlg->GetCalls()->GetCallMap().end(); ++it)
			{
				...
				CString     strRegURL;
				strRegURL.Format(_T("sip:%s:%d"), it->second.domain, it->second.port);

				strURL.Format(_T("sip:%s@%s:%d"), it->second.name, it->second.domain, it->second.port);
				AddTransportSuffix(strURL);
				strURL = _T("<") + strURL + _T(">");

				acc_cfg.id = StrToPjStr(strURL);
				acc_cfg.reg_uri = StrToPjStr(strRegURL);
				acc_cfg.cred_count = 1;
				acc_cfg.cred_info[0].realm = StrToPjStr(it->second.domain);
				acc_cfg.cred_info[0].scheme = pj_str("Digest");
				acc_cfg.cred_info[0].username = StrToPjStr(it->second.name);
				acc_cfg.cred_info[0].data_type = PJSIP_CRED_DATA_PLAIN_PASSWD;
				acc_cfg.cred_info[0].data = StrToPjStr(it->second.passwd);
				acc_cfg.reg_timeout = it->second.regtimeout;
				transport = MSIP_TRANSPORT_AUTO;
				if (accountSettings.account.transport == _T("udp") && transport_udp != -1) {
					acc_cfg.transport_id = transport_udp;
				}
				...
				pj_status_t status = PJ_SUCCESS;
				status = pjsua_acc_add(&acc_cfg, PJ_TRUE, &cur_acc_id);
				if (status == PJ_SUCCESS)
				{
					it->second.accid = cur_acc_id;
				}
				else
				{
...
void CmainDlg::PlayerMultiplePlay(std::vector<CString> strFileList)
{
	...
	if (player_multiple_id == PJSUA_INVALID_ID && pjsua_var.state != PJSUA_STATE_NULL)
	{
		...
		unsigned i, count = 0;
		pj_str_t pj_lbl = StrToPjStr(CString("PlayerMultiplePlay"));
		pj_status_t status;
...
void CmainDlg::PlayerMultipleStop( )
{
	if (player_multiple_id != PJSUA_INVALID_ID) {
		if (pjsua_var.state != PJSUA_STATE_NULL) {
			pjsua_conf_disconnect(pjsua_player_get_conf_port(player_multiple_id), 0);
			if (pjsua_player_destroy(player_multiple_id) == PJ_SUCCESS) {
				player_multiple_id = PJSUA_INVALID_ID;
			}
		}
		else {
...
LRESULT CmainDlg::onMultipleIncomingCall(WPARAM wParam, LPARAM lParam) 
{
	...
	ParseSIPURI(info, &remotesipuri);

	info = PjToStr(&call_info.local_info, TRUE);

	ParseSIPURI(info, &localsipuri);
...
void CmainDlg::CheckUpdates()
{
	...
	try {
		CHttpFile* pFile;
		CString url = _T("http://update.seami.cloud/?version=");
		...
		if (pFile)
		{
			...
			if (dwStatusCode == 202) {
				...
				{
					OpenURL(_T("http://www.seami.cloud/downloads"));
				}
			}
			pFile->Close();
		}
...
void  CmainDlg::GetPlayFileList(CString strPath,std::vector<CString>& FileList)
{
	...
	HANDLE hError;
	CString strFilePath = strPath + CString("\\");
	CString strFilePathName = strFilePath + CString("*.*");
	int FileCount = 0;

	hError = FindFirstFile(strFilePathName, &FindData);
	...
	while (::FindNextFile(hError, &FindData))
	{
		...
		FileList.push_back(strFilePath + FindData.cFileName);
		FileCount++;
	}
