...
void MessagesDlg::OnChangeTab(pjsua_call_info *p_call_info, call_user_data *user_data)
{
	...
	if (messagesContact->callId != -1) {
		...
		if (accountSettings.singleMode
			...
			{
				mainDlg->GetDialer()->SetNumber(!sipuri.user.IsEmpty() && sipuri.domain == accountSettings.account.domain ? sipuri.user : messagesContact->number, 1);
			}
		}
	} else {
		UpdateCallButton();
		if (accountSettings.singleMode && mainDlg->GetDialer()) {
			mainDlg->GetDialer()->PostMessage(WM_COMMAND,MAKELPARAM(IDC_CLEAR,0),0);
		}
	}
...
void MessagesDlg::UpdateCallButton(BOOL active, pjsua_call_info *call_info)
{
	...
	if (!active) {
		if (mainDlg->GetTransferDlg()) {
			mainDlg->GetTransferDlg()->OnClose();
		}
...
void MessagesDlg::UpdateHoldButton(pjsua_call_info *call_info)
{
	...
	CButton* buttonHoldDialer = mainDlg->GetDialer() ? (CButton*)mainDlg->GetDialer()->GetDlgItem(IDC_HOLD) : NULL;
	CButton* buttonTransferDialer = mainDlg->GetDialer() ? (CButton*)mainDlg->GetDialer()->GetDlgItem(IDC_TRANSFER) : NULL;
...
{
	MessagesContact* messagesContactSelected = mainDlg->GetMessagesDlg() ?  mainDlg->GetMessagesDlg()->GetMessageContact() : NULL;
	...
	if (!number.IsEmpty()) {
		...
		if (action == MSIP_ACTION_INVITE) {
			MessagesContact *messagesContact =	mainDlg->GetMessagesDlg() ?  mainDlg->GetMessagesDlg()->AddTab(FormatNumber(number), _T(""), TRUE, NULL, NULL, TRUE) : NULL;
			if (messagesContact->callId == -1) {
				pjsua_call_info call_info;
				...
				user_data->inConference = true;
				if(mainDlg->GetMessagesDlg())
					mainDlg->GetMessagesDlg()->CallStart(false, user_data);
			}
		}
	}
}
...
{
	if (!mainDlg->GetTransferDlg()) {
		mainDlg->SetTransferDlg(new Transfer(this));
	}
	mainDlg->GetTransferDlg()->SetAction(MSIP_ACTION_TRANSFER);
	mainDlg->GetTransferDlg()->SetForegroundWindow();
}
...
void MessagesDlg::OnConference()
{
	...
	mainDlg->GetTransferDlg()->SetAction(MSIP_ACTION_INVITE);
	mainDlg->GetTransferDlg()->SetForegroundWindow();
}
