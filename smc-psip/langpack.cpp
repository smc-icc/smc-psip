...
// MurmurHash2 
unsigned int __fastcall myhash(const void * key, unsigned int len)
{
...
static int LoadLangPack(const TCHAR *szLangPack)
{
	...
	TrimString(line);
	if(lstrcmpA(line,"Language Pack")) {fclose(fp); return 2;}
	//headers
	while(!feof(fp)) {
		...
		*pszColon=0;
		...
		else if(!lstrcmpA(line,"RTL")) {
			_snprintf(line,sizeof(line),"%s",pszColon+1);
			TrimString(line);
			langPack.rtl=!lstrcmpA(line,"1");
		}
...
void TranslateMenu( HMENU hmenu )
{
	...
	{
		if (GetMenuString( hmenu, i, title, 2048, MF_BYPOSITION ))
		{
			wchar_t * p = wcsstr(title ,_T("\t"));
			TCHAR translated[2048] = _T("");
			if (p!=NULL) {
				*p = 0;
			}
			StringCchCat(translated,2048,(LPTSTR)LangPackTranslateString((char*)title, FLAGS));
			if (p!=NULL) {
				*p = '\t';
				StringCchCat(translated,2048,p);
			}
			ModifyMenu(
			hmenu,
			i,
