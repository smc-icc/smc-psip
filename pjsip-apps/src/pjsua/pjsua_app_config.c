...
struct pj_getopt_option long_options[] = {
	...
	{ "mutil-reg-cfg", 1, 0, OPT_MUTIL_REG_CFG },
	{ NULL, 0, 0, 0 }
};
...
static void usage(void)
{
	...
	puts("  --mutil-reg-cfg     register account from cfg");
	puts("");
...
* Read command arguments from config file.
*/
static int read_mutil_reg_cfg_file(pjsua_app_config *cfg, const char *filename)
{
	int i,c;
	FILE *fhnd;
	char line[200];
	...
	fhnd = fopen(filename, "rt");
	if (!fhnd) {
		PJ_LOG(1, (THIS_FILE, "Unable to open mutil_reg_cfg file %s", filename));
		fflush(stdout);
		return -1;
	}
	...
	while (!feof(fhnd)) {
		...
		for (p = line; *p != '\0'; p++) {
			pj_bzero(szToken, sizeof(szToken));
			pj_bzero(szOptarg, sizeof(szOptarg));
			// first, scan whitespaces
			while (*p != '\0' && strchr(whitespace, *p) != NULL) p++;
			...
			if (token_len > 2) {
				...
				strncpy(szToken, token+2, token_len-2);
				for (i = 0;i < OPT_MUTIL_REG_CFG - OPT_CONFIG_FILE;i++)
				{
					if (!strncmp(szToken,long_options[i].name,token_len - 2))
					{
						c = long_options[i].val;
						if (c == OPT_NEXT_ACCOUNT)
							goto   NextAccount;
						optarg = p + 1;
				...
				optarg_len = (int)(p - optarg);
				strncpy(szOptarg, optarg, optarg_len);
				switch (c) {
				case OPT_ID:   /* id */
					if (pjsua_verify_url(szOptarg) != 0) {
						PJ_LOG(1, (THIS_FILE,
							"Error: invalid SIP URL '%s' "
							"in local id argument", szOptarg));
						return PJ_EINVAL;
					}
					cur_acc->id = pj_strdup3(cfg->pool,szOptarg);
					break;
				case OPT_REGISTRAR:   /* registrar */
					if (pjsua_verify_sip_url(szOptarg) != 0) {
						PJ_LOG(1, (THIS_FILE,
							"Error: invalid SIP URL '%s' in "
							"registrar argument", szOptarg));
						return PJ_EINVAL;
					}
					cur_acc->reg_uri = pj_strdup3(cfg->pool, szOptarg);
					break;
				case OPT_REALM:	    /* Default authentication realm. */
					cur_acc->cred_info[cur_acc->cred_count].realm = pj_strdup3(cfg->pool, szOptarg);
					break;
				case OPT_USERNAME:   /* Default authentication user */
					cur_acc->cred_info[cur_acc->cred_count].username = pj_strdup3(cfg->pool, szOptarg);
					cur_acc->cred_info[cur_acc->cred_count].scheme = pj_strdup3(cfg->pool, "Digest");
					break;
				case OPT_PASSWORD:   /* authentication password */
					cur_acc->cred_info[cur_acc->cred_count].data_type = PJSIP_CRED_DATA_PLAIN_PASSWD;
					cur_acc->cred_info[cur_acc->cred_count].data = pj_strdup3(cfg->pool, szOptarg);
#if PJSIP_HAS_DIGEST_AKA_AUTH
					cur_acc->cred_info[cur_acc->cred_count].data_type |= PJSIP_CRED_DATA_EXT_AKA;
					cur_acc->cred_info[cur_acc->cred_count].ext.aka.k = pj_strdup3(cfg->pool, szOptarg);;
	...
	pj_str_t *uri_to_call)
{
	...
	while ((c = pj_getopt_long(argc, argv, "", long_options, &option_index)) != -1) {
		...
		case OPT_DISABLE_CLI_CONSOLE:
			...
			break;
		case OPT_MUTIL_REG_CFG:
			read_mutil_reg_cfg_file(cfg,pj_optarg);
			break;
		default:
			PJ_LOG(1, (THIS_FILE,
